...
<%
	...
	if (request.getMethod().equals("GET")) {
		...
		if (s1 == null || s1.length() == 0) 
        	...
        	response.sendRedirect("studentList1.jsp?pg=" + pg + "&ss=" + ss + "&st=" + srchTextEncoded + "&od=" + od);
			return;
    	}
	}
%>
...
<div class="container">
	...
	<form method="post">
		...
		<a href="studentDelete1.jsp?id=<%= id %>&pg=<%= pg %>&ss=<%=ss %>&st=<%= srchTextEncoded %>&od=<%=od%>"
			class="btn btn-danger"onclick="return confirm('삭제하시겠습니까?')">
			<i class="glyphicon glyphicon-trash"></i>삭제
		</a>
		<a href="studentList1.jsp?pg=<%= pg %>&ss=<%=ss %>&st=<%= srchTextEncoded %>&od=<%=od%>"
