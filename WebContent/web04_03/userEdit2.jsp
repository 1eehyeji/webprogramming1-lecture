...
<%
	...
	if (request.getMethod().equals("GET")) {
    	...
    	boolean b1 = request.getParameter("enabled") != null ? true : false;
    	user.setEnabled(b1);
		...
		if (user.getUserid() == null || user.getUserid().length() == 0) 
        	...
        	UserDao.update(user);
        	response.sendRedirect("userList1.jsp?pg=" + pg + "&ss=" + ss + "&st=" + srchTextEncoded + "&od=" + od);
			return;
    	}
	}
%>
...
<div class="container">
	...
	<form method="post">
		...
		<div class="form-group" style="margin-left: 20px;">
			<div class="checkbox">
        		<input type="checkbox" name="enabled" value="1" <%= user.isEnabled() ? "checked" : "" %>> 동의여부 <br/>
			</div>
		</div>
		...
		<a href="userDelete1.jsp?id=<%= id %>&pg=<%= pg %>&ss=<%=ss %>&st=<%= srchTextEncoded %>&od=<%=od%>"
			class="btn btn-danger" onclick="return confirm('삭제하시겠습니까?')">
			<i class="glyphicon glyphicon-trash"></i>삭제
		</a>
		
		<a href="userList1.jsp?pg=<%= pg %>&ss=<%=ss %>&st=<%= srchTextEncoded %>&od=<%=od%>"
